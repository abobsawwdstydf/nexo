...
export default function NexoAIPage({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <div className="h-full flex flex-col relative">
      ...
      {/* ====== HEADER ====== */}
      <div className="px-4 py-3 flex items-center gap-3 flex-shrink-0 relative z-10" style={{ background: '#1F1F23', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        {/* Закрыть (мобилки) */}
        {isMobile && onClose && (
          <button
            onClick={onClose}
              ...
              {messages.map((msg) => (
                ...
                >
                  <div
                    ...
                    style={msg.role !== 'user' ? { background: '#2B2B2F', border: '1px solid #1F1F23' } : undefined}
                  >
      ...
      <div className="px-3 py-3 flex-shrink-0 relative z-10">
        <div className="rounded-2xl px-3 py-2 flex items-end gap-2 transition-colors" style={{ background: '#1F1F23', border: '1px solid rgba(255,255,255,0.08)' }}>
          {/* Голос */}
          <button
