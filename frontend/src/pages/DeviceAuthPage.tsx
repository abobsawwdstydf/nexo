...
import { Check, X, Smartphone, Loader2, Mail, ArrowLeft } from 'lucide-react';
...
export default function DeviceAuthPage() {
  const { user, sendLoginCode, loginConfirm, loginWithToken } = useAuthStore();
        ...
        <AuthCard className="max-w-sm w-full mx-4">
          <div className="text-center">
            ...
            <ShimmerButton
              ...
              className="mt-6 w-full py-3 px-4 rounded-xl bg-gradient-to-r from-[#7B61FF] to-[#8b5cf6] text-white font-semibold text-[14px] flex items-center justify-center gap-2 relative overflow-hidden"
  ...
  if (!user) {
    ...
    const handleConfirmCode = async () => {
      ...
      setLoginError('');
      try {
        await loginConfirm(loginEmail, loginCode);
        setAction('idle');
        if (deviceToken) {
        ...
        <AuthCard className="max-w-sm w-full mx-4">
          <div className="text-center">
            ...
            <h1
              ...
              style={{
                background: 'linear-gradient(135deg, #ffffff 0%, #cdbdff 50%, #7B61FF 100%)',
                WebkitBackgroundClip: 'text',
                WebkitTextFillColor: 'transparent',
                backgroundClip: 'text',
            ...
            <p className="text-white/40 text-[13px] mt-2 mb-5">
              {loginStep === 'email'
                ...
                : `Код отправлен на ${loginEmail}`}
            </p>

            <div className="space-y-3 text-left">
              {loginStep === 'email' ? (
                <div className="relative">
                  <span className="text-[12px] text-white/40 ml-1 mb-1 block">Email</span>
                  <div className="relative">
                    <div className="absolute left-3 top-1/2 -translate-y-1/2 z-10">
                      <Mail size={20} className="text-[#7B61FF]" />
                    </div>
                    <input
                      type="email"
                      value={loginEmail}
                      ...
                      className="w-full pl-11 pr-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder-white/25 focus:border-[#7B61FF]/50 focus:bg-white/[0.06] focus:ring-2 focus:ring-[#7B61FF]/20 transition-all duration-300 outline-none text-sm"
                    />
                  </div>
                </div>
              ) : (
                <div className="relative">
                  <span className="text-[12px] text-white/40 ml-1 mb-1 block">Код подтверждения</span>
                  <div className="relative">
                    <div className="absolute left-3 top-1/2 -translate-y-1/2 z-10">
                    ...
                    <input
                      ...
                      placeholder="000000"
                      maxLength={6}
                      className="w-full pl-11 pr-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder-white/25 focus:border-[#7B61FF]/50 focus:bg-white/[0.06] focus:ring-2 focus:ring-[#7B61FF]/20 transition-all duration-300 outline-none text-sm tracking-[0.3em] text-center font-mono"
                    />
                  </div>
                </div>
              )}
              ...
              {loginStep === 'code' && (
                <div className="flex items-center justify-between text-xs">
                  ...
                  <button
                    type="button"
                    onClick={() => { if (resendTimer === 0) { handleSendCode(); } }}
                    disabled={resendTimer > 0}
                    className={`${resendTimer > 0 ? 'text-white/20' : 'text-[#7B61FF] hover:text-[#9b82ff]'} transition-colors`}
                  >
              ...
              <ShimmerButton
                ...
                whileTap={{ scale: 0.98 }}
                onClick={loginStep === 'email' ? handleSendCode : handleConfirmCode}
                disabled={loginLoading || (loginStep === 'email' ? !loginEmail : loginCode.length !== 6)}
                className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-[#7B61FF] to-[#8b5cf6] text-white font-semibold text-[14px] flex items-center justify-center gap-2 relative overflow-hidden disabled:opacity-50"
                style={authPrimaryButtonStyle}
              >
                {loginLoading
                  ? <Loader2 size={18} className="animate-spin" />
                  : loginStep === 'email'
                    ? <><Mail size={18} /> Получить код</>
                    : <><Check size={18} /> Войти</>
                }
              </ShimmerButton>
            </div>
        ...
        <div className="text-center">
          <div className="flex justify-center mb-5">
            <motion.div
              ...
              className="relative"
            >
              <div className="absolute inset-0 -m-3 rounded-[1.4rem] bg-[#7B61FF]/30 blur-xl" />
              <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                transition={{ type: 'spring', bounce: 0.4 }}
                className="relative w-20 h-20 rounded-[1.1rem] bg-gradient-to-br from-[#7B61FF]/25 to-[#8b5cf6]/25 border border-[#7B61FF]/30 flex items-center justify-center"
