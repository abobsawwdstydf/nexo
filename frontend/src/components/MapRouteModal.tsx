...
export default function MapRouteModal({ onClose, onSend }: MapRouteModalProps) {
  ...
  const initMap = (lat: number, lng: number) => {
    ...
    const icon = L.divIcon({ html: '<div style="width:16px;height:16px;background:#7B61FF;border-radius:50%;border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.5)"></div>', iconSize: [16, 16], iconAnchor: [8, 8] });
  ...
  const handleSearchDest = async () => {
    ...
    if (L && map) {
      ...
      if (myLoc) {
        ...
        routeLayerRef.current = L.polyline([[myLoc.lat, myLoc.lng], [result.lat, result.lng]], { color: '#7B61FF', weight: 3, dashArray: '8 4' }).addTo(map);
      }
    }
  };
