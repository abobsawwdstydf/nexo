...
import { Phone, PhoneOff, Video, VideoOff, Mic, MicOff, Monitor, MonitorOff, Minimize2, Volume2, ShieldCheck, ShieldOff, ChevronUp, Film, Tv } from 'lucide-react';
...
export default function GroupCallModal({ isOpen, onClose, chatId, chatName, callType: initialCallType }: GroupCallModalProps) {
  ...
  useEffect(() => {
    ...
    const onVideoShare = (data: { chatId: string; from: string; fileName?: string; action: 'start' | 'stop' }) => {
      if (data.chatId !== chatId) return;
      if (data.action === 'stop') {
    ...
    const onVideoSync = (data: { chatId: string; from: string; action: 'play' | 'pause' | 'seek'; currentTime: number }) => {
      if (data.chatId !== chatId || !sharedVideoRef.current) return;
      if (Math.abs(sharedVideoRef.current.currentTime - data.currentTime) > 1) {
        sharedVideoRef.current.currentTime = data.currentTime;
      }
      if (data.action === 'play') sharedVideoRef.current.play().catch(() => {});
      if (data.action === 'pause') sharedVideoRef.current.pause();
    };

    // Новые события через video_share_*
    const onVideoShareStarted = (data: { from: string; videoUrl: string }) => {
      setRemoteVideoShareUrl(data.videoUrl);
      setShowVideoShareModal(true);
    };
    const onVideoShareStopped = () => {
      setRemoteVideoShareUrl(null);
    };
    const onVideoShareSync = (data: { from: string; currentTime: number; isPlaying: boolean }) => {
      setVideoShareTime(data.currentTime);
      setVideoSharePlaying(data.isPlaying);
    };

    socket.on('group_call_video_share', onVideoShare);
    socket.on('group_call_video_sync', onVideoSync);

    socket.on('video_share_started', onVideoShareStarted);
    socket.on('video_share_stopped', onVideoShareStopped);
    socket.on('video_share_sync', onVideoShareSync);
    ...
    return () => {
      ...
      socket.off('group_call_video_share', onVideoShare);
      socket.off('group_call_video_sync', onVideoSync);
      socket.off('video_share_started', onVideoShareStarted);
      socket.off('video_share_stopped', onVideoShareStopped);
      socket.off('video_share_sync', onVideoShareSync);
    };
  ...
  return (
      ...
      {isMinimized && joined ? (
        ...
        >
          ...
          >
              ...
              <button
                onClick={() => {
                  if (isVideoSharing) {
                    ...
                    setIsVideoSharing(false);
                    const socket = getSocket();
                    if (socket) socket.emit('group_call_video_share', { chatId, action: 'stop' });
                    if (socket) socket.emit('video_share_stop', { chatId });
                  } else {
                    setShowVideoShareModal(true);
                  }
                }}
                className={`w-11 h-11 rounded-full flex items-center justify-center transition-colors ${isVideoSharing ? 'bg-nexo-500/30 text-nexo-400' : 'bg-white/10 text-white hover:bg-white/20'}`}
                title={isVideoSharing ? 'Остановить трансляцию' : 'Транслировать видео'}
              >
                <Film size={18} />
              </button>
              {/* Watch Party button */}
            ...
            {sharedVideoUrl && (
              <div className="px-8 pb-6">
                <div className="relative rounded-2xl overflow-hidden bg-black border border-white/10">
                  <div className="absolute top-2 left-2 z-10 px-2 py-1 rounded-lg bg-nexo-500/80 text-xs text-white font-medium flex items-center gap-1">
                    <Film size={12} /> Совместный просмотр
                  </div>
                  <button
                    onClick={() => { setSharedVideoUrl(null); setIsVideoSharing(false); }}
                  ...
                  <video
                    ...
                    onPlay={() => {
                      const socket = getSocket();
                      if (socket && sharedVideoRef.current) {
                        socket.emit('group_call_video_sync', {
                          chatId,
                          action: 'play',
                          currentTime: sharedVideoRef.current.currentTime,
                        });
                      }
                    }}
                    onPause={() => {
            ...
            {/* Hidden file input for video sharing */}
            <input
              ...
              onChange={(e) => {
                ...
                const socket = getSocket();
                if (socket) {
                  socket.emit('group_call_video_share', {
                    chatId,
                    fileName: file.name,
                    action: 'start',
                  });
    ...
    <VideoShareModal
      ...
      onShare={(url, type) => {
        ...
        const socket = getSocket();
        if (socket) {
          socket.emit('video_share_start', { chatId, videoUrl: url, type });
        }
      }}
      ...
      onTimeUpdate={(time) => {
        const socket = getSocket();
        if (socket) socket.emit('video_share_sync', { chatId, currentTime: time, isPlaying: videoSharePlaying });
      }}
      onPlayPause={(playing) => {
        setVideoSharePlaying(playing);
        const socket = getSocket();
        if (socket) socket.emit('video_share_sync', { chatId, currentTime: videoShareTime, isPlaying: playing });
      }}
      isMine={isVideoSharing}
    />
