...
const STORY_BG_COLORS = [
  '#7B61FF', '#8b5cf6', '#ec4899', '#f43f5e', '#ef4444',
      ...
      >
        ...
        {currentStory.type === 'video' && currentStory.mediaUrl ? (
          ...
          <div
            ...
            style={{ background: currentStory.bgColor || '#7B61FF' }}
          >
...
export function CreateStoryModal({ onClose, onCreated }: CreateStoryModalProps) {
  ...
  const [bgColor, setBgColor] = useState('#7B61FF');
    ...
    >
      ...
      >
        <div className="p-4 border-b border-white/10 flex items-center justify-between">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-full flex items-center justify-center text-zinc-400 hover:text-white bg-white/10 backdrop-blur-sm border border-white/10 hover:bg-white/20 transition-all">
            <X size={18} />
          </button>
        </div>
