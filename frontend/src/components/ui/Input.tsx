  ...
  (props, ref) => {
    ...
    return (
      <div className="relative w-full">
        ...
        <motion.div
          className="absolute -inset-0.5 rounded-full bg-gradient-to-r from-nexo-500/20 via-nexo-400/10 to-nexo-500/20 blur-sm pointer-events-none"
          animate={{ opacity: isFocused ? 1 : 0, scale: isFocused ? 1 : 0.98 }}
          transition={{ duration: 0.3, ease: 'easeOut' }}
        />
        <div className="relative flex items-center gap-2.5 bg-white/[0.04] rounded-full px-4 py-2.5 border border-white/[0.06] focus-within:border-nexo-500/40 focus-within:bg-white/[0.06] transition-all duration-300">
          <motion.div
            animate={{ rotate: isFocused ? 90 : 0, scale: isFocused ? 1.1 : 1 }}
            transition={{ type: 'spring', stiffness: 300, damping: 20 }}
          >
            <Search size={16} className={cn('transition-colors duration-300', isFocused ? 'text-nexo-400' : 'text-zinc-500')} />
          </motion.div>
          <input
            ref={ref}
