...
import { getSocket, wsEmit, wsRequest } from '../lib/socket';
...
export default function ChatView({ onStartCall, onStartGroupCall }: { onStartCall?: (targetUser: UserBasic, type: 'voice' | 'video') => void; onStartGroupCall?: (chatId: string, chatName: string, type: 'voice' | 'video') => void }) {
  ...
  useEffect(() => {
    ...
    if (unread.length > 0) {
      ...
      // Send read receipts via WebSocket RPC
      wsRequest('read_receipt', { chatId: activeChat, messageId: ids[0] }).catch(() => {});
    ...
    >
      ...
      {selectionMode ? (
              ...
              <AnimatePresence>
                {showDeleteMenu && (
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-2 w-56 rounded-2xl glass-strong shadow-2xl z-50 py-1.5 ring-1 ring-border/50 overflow-hidden"
                  >
                    <button
        ...
        <div className="h-[60px] sm:h-[76px] flex items-center justify-between px-2 sm:px-4 z-20 flex-shrink-0 relative">
          ...
          <motion.button
            ...
            aria-label="Назад"
            className="relative z-10 w-[44px] h-[44px] rounded-2xl flex items-center justify-center transition-all hover:scale-105 active:scale-95 shrink-0 glass"
          >
          ...
          <div
            className="absolute left-1/2 -translate-x-1/2 z-10 flex items-center gap-1 pl-5 pr-1 py-1 rounded-full transition-all min-w-[200px] max-w-[calc(100vw-120px)] sm:max-w-[340px] glass"
          >
            <button
              onClick={() => {
            ...
            >
              ...
              <span className="text-[10px] sm:text-[11px] leading-tight mt-0.5 w-full truncate text-white/50">
                {isFavorites
          ...
          <div className="relative z-10 shrink-0">
            <button
              ...
              className="w-[44px] h-[44px] rounded-full overflow-hidden flex items-center justify-center transition-all hover:scale-105 active:scale-95 glass"
            >
              {isFavorites ? (
      ...
      <AnimatePresence>
        {showSearch && (
          <motion.div
            ...
            className="absolute top-[60px] sm:top-[76px] left-0 right-0 z-20 max-h-[300px] overflow-y-auto glass-strong border-b border-border"
          >
            {isSearching ? (
      ...
      {pinnedMsg && (
        <button
          ...
          className="flex items-center gap-3 px-4 py-2 border-b border-border glass-subtle hover:bg-surface-hover transition-colors text-left w-full flex-shrink-0"
        >
