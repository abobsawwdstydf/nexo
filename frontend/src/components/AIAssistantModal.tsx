...
import { X, Send, Loader2, Bot, Paperclip, Mic } from 'lucide-react';
...
export default function AIAssistantModal({ 
    ...
    <div className="flex flex-col h-full max-h-[60vh] min-h-[300px]">
      ...
      <div className="border-t border-white/5 p-4">
        <div className="flex items-end gap-2">
          <button className="flex items-center justify-center w-9 h-9 rounded-full hover:bg-white/10 transition-colors flex-shrink-0 mb-0.5">
            <Paperclip size={20} className="text-white/50 rotate-[-45deg]" />
          </button>
          <div className="flex-1 relative">
            <textarea
              ...
              placeholder={messageContext ? "Что вы хотите узнать об этом сообщении?" : "Сообщение..."}
              className="w-full bg-white/5 border border-white/10 rounded-2xl px-3.5 py-2 text-sm text-white placeholder-zinc-500 resize-none focus:outline-none focus:border-[#845EF7]/40 transition-colors"
              rows={1}
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-[#845EF7] hover:bg-[#7049e8] disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center flex-shrink-0"
          >
            {loading ? (
  ...
  if (isMobile) {
    return (
      <BottomSheet isOpen onClose={onClose} showCloseButton={false} maxHeight="85vh">
        {header}
        {content}
      </BottomSheet>
    );
