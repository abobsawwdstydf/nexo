...
.t-clear-btn:hover { opacity: 1; background: rgba(255, 255, 255, 0.15); }
...
export const ClearInput = forwardRef<ClearInputHandle, ClearInputProps>(
  ({ value, onChange, onClear, leftIcon, className = "", placeholder = "", defaultValue, ...rest }, ref) => {
    ...
    return (
      <div ref={wrapRef} className={`t-clear rounded-full ${hasVal ? "has-value" : ""} ${leftIcon ? "has-icon" : ""} ${className}`}>
        {leftIcon && (
