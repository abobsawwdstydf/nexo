    ...
    <div className="flex flex-col h-full liquid-glass">
            ...
            <AnimatePresence>
              {showProfileMenu && (
                <motion.div
                  ...
                  className="absolute right-0 top-full mt-2 w-56 rounded-2xl glass-strong py-1.5 z-50 overflow-hidden"
                >
                  {isSelf ? (
                    <>
                      <button
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden pb-24 sm:pb-8">
        ...
        <div className="relative h-[180px] w-full overflow-hidden flex-shrink-0">
          ...
          <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-[#0a0a0f]/90" />
        ...
        <div className="flex flex-col items-center -mt-[60px] pb-3 px-6 relative z-10">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[200px] h-[200px] bg-[#7B61FF]/8 rounded-full blur-[60px] pointer-events-none" />
          ...
          >
            ...
            <div className={`relative w-[120px] h-[120px] rounded-full overflow-hidden ${isPremium ? 'ring-[3px] ring-yellow-400/60' : ''}`}
              style={!isPremium ? { border: '3px solid #1F1F1F', boxShadow: '0 0 20px rgba(123,97,255,0.1)' } : undefined}
            >
              {profile.avatar ? (
                <img
                  src={profile.avatar}
        ...
        <div className="mx-4 mb-3 rounded-2xl glass-card">
          {/* Bio */}
          ...
          <div className="mx-4 mb-3 rounded-2xl glass-card">
            ...
            >
              {profile.pinnedChannel.avatar ? (
                <img src={profile.pinnedChannel.avatar} alt="" className="w-10 h-10 rounded-full object-cover flex-shrink-0" />
              ) : (
                <div className="w-10 h-10 rounded-full bg-nexo-500/20 flex items-center justify-center flex-shrink-0">
        ...
        <div className="mx-4 mb-3 rounded-2xl overflow-hidden liquid-glass">
          <div className="relative h-[160px] flex items-center justify-center overflow-hidden">
            ...
            <svg viewBox="0 0 120 120" className="w-[80px] h-[80px] opacity-20" style={{ animation: 'spin3d 12s linear infinite' }}>
              <ellipse cx="60" cy="60" rx="50" ry="20" fill="none" stroke="#7B61FF" strokeWidth="0.8" opacity="0.6" />
              <ellipse cx="60" cy="60" rx="50" ry="20" fill="none" stroke="#7B61FF" strokeWidth="0.8" opacity="0.6" transform="rotate(60 60 60)" />
              <ellipse cx="60" cy="60" rx="50" ry="20" fill="none" stroke="#7B61FF" strokeWidth="0.8" opacity="0.6" transform="rotate(120 60 60)" />
              <circle cx="60" cy="60" r="4" fill="#7B61FF" opacity="0.5" />
            </svg>
            <style>{`
            ...
            <div className="absolute bottom-3 left-0 right-0 text-center">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-medium" style={{ background: 'rgba(123,97,255,0.15)', color: '#7B61FF', border: '1px solid rgba(123,97,255,0.2)' }}>
              ...
              {profileNFTCards.length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-3">
                  {profileNFTCards.map((instance: any) => (
                    <div key={instance.id} className="rounded-2xl overflow-hidden border border-white/[0.08] glass-card">
              ...
              {profileNFTTags.length === 0 ? (
                ...
                <div className="grid grid-cols-3 gap-3">
                  {profileNFTTags.map((instance: any) => (
                    <div key={instance.id} className="glass-card rounded-2xl p-3 text-center">
              ...
              {userHashtags.length === 0 ? (
                ...
                <div className="space-y-2">
                  {userHashtags.map((hashtag) => (
                    <div key={hashtag.id} className="glass-card rounded-xl p-3 hover:bg-white/[0.08] transition-colors cursor-pointer"
        ...
        {isEditingProfile && (
          <>
            ...
            <motion.div
              ...
              className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md glass-strong rounded-2xl z-50 shadow-2xl overflow-hidden"
            >
              ...
              <div className="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={editData.username} onChange={(e) => setEditData({ ...editData, username: e.target.value })}
                      className="w-full pl-10 pr-4 py-3 rounded-xl glass-input text-white focus:border-nexo-500 focus:outline-none text-[14px]"
                ...
                <div>
                  ...
                  <input type="text" value={editData.displayName} onChange={(e) => setEditData({ ...editData, displayName: e.target.value })}
                    className="w-full px-4 py-3 rounded-xl glass-input text-white focus:border-nexo-500 focus:outline-none text-[14px]"
                ...
                <div>
                  ...
                  <textarea value={editData.bio} onChange={(e) => setEditData({ ...editData, bio: e.target.value })}
                    className="w-full px-4 py-3 rounded-xl glass-input text-white focus:border-nexo-500 focus:outline-none resize-none h-24 text-[14px]"
              ...
              <div className="flex gap-3 p-4 border-t border-white/[0.06]">
                ...
                <motion.button whileTap={{ scale: 0.97 }} onClick={() => setIsEditingProfile(false)}
                  className="flex-1 py-3 rounded-xl glass-btn text-white font-medium transition-all text-[14px]">
                  Отмена
                </motion.button>
              </div>
            </motion.div>
        ...
        {isEditingStatus && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div
              ...
              className="relative w-full max-w-md rounded-2xl glass-strong shadow-2xl overflow-hidden"
              onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-4 space-y-4">
                <div>
                  ...
                  <input type="text" value={statusEmoji} onChange={(e) => setStatusEmoji(e.target.value.slice(0, 2))}
                    ...
                    className="w-full px-3 py-2.5 rounded-xl glass-input text-white text-2xl text-center placeholder-zinc-500 focus:outline-none focus:border-nexo-500 transition-colors" />
                </div>
                <div>
                  ...
                  <input type="text" value={statusText} onChange={(e) => setStatusText(e.target.value.slice(0, 200))}
                    ...
                    className="w-full px-3 py-2.5 rounded-xl glass-input text-white placeholder-zinc-500 focus:outline-none focus:border-nexo-500 transition-colors text-[14px]" />
                ...
                <div className="flex gap-2 pt-2">
                  <button onClick={handleSaveStatus} disabled={!statusText.trim()}
                    className="flex-1 py-2.5 rounded-xl bg-nexo-500 hover:bg-nexo-600 disabled:glass-btn disabled:text-zinc-500 text-white font-medium transition-colors text-[14px]">
                    Сохранить
                  </button>
                  {status && (
                  ...
                  <button onClick={() => setIsEditingStatus(false)}
                    className="px-4 py-2.5 rounded-xl glass-btn text-zinc-500 hover:text-white font-medium transition-colors text-[14px]">
                    Отмена
                  </button>
                </div>
              </div>
  ...
  return (
    <>
      ...
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-md hidden sm:block"
        onClick={onClose}
      />
      {/* Modal container — full-screen on mobile, centered card on desktop */}
      <div className="fixed inset-0 sm:inset-auto sm:top-1/2 sm:left-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:w-[440px] sm:max-w-[calc(100vw-48px)] sm:h-[82vh] sm:max-h-[780px] z-[101] flex flex-col sm:rounded-[2rem] overflow-hidden animate-[slideUp_0.2s_ease-out] sm:animate-none"
