...
export default function SideMenu({ isOpen, onClose }: SideMenuProps) {
              ...
              <div className="relative">
                {user?.avatar ? (
                  <img src={user.avatar} alt="" className="w-[64px] h-[64px] rounded-full object-cover ring-2 ring-white/10" />
                    ...
                    <div className="w-[64px] h-[64px] rounded-full bg-white/10 flex items-center justify-center ring-2 ring-white/10 relative overflow-hidden backdrop-blur-sm">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto py-2">
        <div className="px-5 py-3">
          ...
          <div className="space-y-1">
            <button
              onClick={() => setLang('ru')}
              className={`w-full flex items-center gap-4 px-3 py-3 rounded-xl transition-colors ${lang === 'ru' ? 'bg-nexo-500/15 ring-1 ring-nexo-500/30' : 'glass-subtle hover:bg-white/[0.06]'}`}
            >
            ...
            <button
              onClick={() => setLang('en')}
              className={`w-full flex items-center gap-4 px-3 py-3 rounded-xl transition-colors ${lang === 'en' ? 'bg-nexo-500/15 ring-1 ring-nexo-500/30' : 'glass-subtle hover:bg-white/[0.06]'}`}
            >
              <span className="text-lg">🇬🇧</span>
              <div className="flex-1 min-w-0 text-left">
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-md z-50" onClick={onClose} />
          <motion.div
