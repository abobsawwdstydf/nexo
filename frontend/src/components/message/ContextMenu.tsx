...
export default function ContextMenu({
  ...
  const handleReaction = (emoji: string) => {
    // Send reaction via WebSocket RPC
    const existingReaction = message.reactions?.find(
      (r) => r.userId === userId && r.emoji === emoji
    );
    // Toggle: if exists, the server will remove it; if not, add it
    wsRequest('reaction', { chatId: message.chatId, messageId: message.id, emoji }).catch(() => {});
    onClose();
  };
