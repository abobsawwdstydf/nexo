...
interface ChatState {
  ...
  setChatsFromInit: (chats: Chat[]) => void;
  ...
  scrollToMessage: (messageId: string, chatId: string) => void;
}
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  drafts: (() => {
    try {
      return loadDecrypted('nexo_drafts') || {};
    } catch {
  ...
  archivedChatIds: (() => {
    try {
      const saved = localStorage.getItem('nexo_archived_chats');
  ...
  setDraft: (chatId, text) => {
    set((state) => {
      ...
      saveEncrypted('nexo_drafts', drafts);
      return { drafts };
    });
  },
  ...
  loadMessages: async (chatId) => {
    try {
      ...
      // Сохраняем в кэш
      messageCache.set(chatId, fetched);
  ...
  addMessage: (message) => {
    ...
    set((state) => {
      ...
      // Обновляем кэш
      messageCache.addMessage(message.chatId, message);
      ...
      saveEncrypted('nexo_chats_cache', updatedChats);
  ...
  scrollToMessage: (messageId: string, chatId: string) => {
    // Store the message to scroll to in a ref that can be accessed from ChatView
    const event = new CustomEvent('nexo:scrollToMessage', { detail: { messageId, chatId } });
  ...
  addChat: (chat) => {
    set((state) => {
      ...
      saveEncrypted('nexo_chats_cache', next);
      return { chats: next };
    });
  },
  ...
  archiveChat: (chatId) => {
    set((state) => {
      const newSet = new Set(state.archivedChatIds);
      newSet.add(chatId);
      localStorage.setItem('nexo_archived_chats', JSON.stringify([...newSet]));
      return { archivedChatIds: newSet };
    });
  },
